    ...
    <section className="flex md:flex-row flex-col-reverse md:py-20 py-10">
      ...
      <div className="flex-1 flex flex-col items-start justify-center md:px-0 px-6">
        <div className="flex flex-col items-center md:items-start py-[6px] px-4 bg-discount-gradient rounded-[10px] mb-2 md:space-y-4 space-y-2 md:px-20 px-10">
          <h2 className="md:text-6xl text-4xl font-bold text-main mb-2">{headerText}</h2>
          <p className="md:text-2xl text-xl text-gray-600">{text}</p>
          <button style={{ backgroundColor: buttonColor }} className="md:text-2xl text-xl text-white font-semibold px-4 py-2 mt-4">
          <Link href="/Contactus">
          {buttonText}
        </Link>
          </button>
